...
COLORS = [(.42,.29,.20),(.48,.56,.28),(.68,.56,.36),(.48,.51,.45),
          (.43,.27,.15),(.48,.35,.23),(.38,.28,.20),(.77,.71,.55),
          (.11,.22,.24),(.23,.34,.15),(.51,.64,.28),(.24,.27,.24),
          (.57,.30,.21),(.30,.45,.42),(.70,.53,.23),(.45,.50,.44)]
N = 1024; tile = N // 4
...
groups={}; collision={}; routes={}; path_surface_count=0
def mesh(group, verts, faces, colors, solid=False,visual_lift=0):
    bucket=groups.setdefault(group,[[],[],[]]); base=len(bucket[0]); bucket[0].extend([(x,y,z+visual_lift) for x,y,z in verts] if visual_lift else verts)
...
def path(g,points,width=3.6,col=2,thick=.18,solid=True,record=None):
    global path_surface_count
    path_surface_count += 1
    ...
    center_base=len(vs); vs.extend([tuple(p) for p in pts])
    fs=[]
    for i in range(len(pts)-1):
        a=i*4;b=a+4
        c=center_base+i
        fs += [(a,b,c+1,c),(c,c+1,b+1,a+1),(a+2,a+3,b+3,b+2),(a,a+2,b+2,b),(a+1,b+1,b+3,a+3)]
    end=(len(pts)-1)*4
    fs += [(0,1,3,2),(end,end+2,end+3,end+1)]
    # Separate coincident visual ribbons at junctions; collision stays on the exact walking surface.
    mesh(g,vs,fs,col,solid,visual_lift=0 if g=='CollisionOnly' else .008+path_surface_count*.002)
...
def bridge(name,a,b,width=4.5,rise_end=1):
    a,b=Vector(a),Vector(b); axis=b-a; side=Vector((-axis.y,axis.x,0)).normalized()
    points=[]
    for i in range(17):
        t=i/16; p=a.lerp(b,t); p.z=a.z+(b.z-a.z)*min(t/rise_end,1)+.38*math.sin(math.pi*t); points.append(tuple(p))
...
path('Paths_Harbour',[(-23,-13,1.25),(-14,-17,1.25),(0,-18,1.25),(15,-16,1.25),(22,-11,1.25)],4.8,record='Harbour promenade')
path('Paths_Village',[(-23,-9,1.25),(-14,-7,1.25),(0,-8,1.25),(13,-7,1.25),(23,-9,1.25)],3.8,record='Village lane')
path('Paths_WestAscent',[(-23,-9,1.25),(-25,-1,3.8),(-22,7,6.25),(-17,10,6.25)],4.5,record='West ascent')
path('Paths_EastAscent',[(23,-9,1.25),(25,-1,3.8),(22,7,6.25),(17,10,6.25)],4.5,record='East ascent')
path('Paths_Upper',[(-19,10,6.25),(-12,8,6.25),(0,7.5,6.25),(13,7,6.25),(20,9,6.25),(20,14,6.25)],4.2,record='Upper terrace')
path('Paths_Summit',[(14,9,6.25),(14,17,8),(11,25,10.2),(7,28,11.25),(-2,26,11.25),(-7,24,11.25),(-7,18,11.25),(-2,16,11.25)],3.8,record='Lighthouse ascent')
bridge('West stone bridge',(-36,15,5.05),(-18,15,6.25))
bridge('East high bridge',(18,18,6.25),(36,18,4.05))
bridge('East harbour bridge',(24,-13,1.25),(36,-13,4.05),rise_end=.7)
path('Paths_Harbour',[(20,-12,1.25),(24,-13,1.25)],3.6)
path('Paths_Headlands',[(-36,-1,5.05),(-38,7,5.05),(-36,15,5.05),(-33,20,5.05)],3.6)
path('Paths_Headlands',[(36,-13,4.05),(38,0,4.05),(37,9,4.05),(36,18,4.05),(35,23,4.05)],3.6)

# Earth beneath the routes makes the inclines read as terrain rather than floating ramps.
for route_name,bottom in [('West ascent',.8),('East ascent',.8),('Lighthouse ascent',6.0)]:
    r=routes[route_name]; pts=[Vector(p) for p in r['points']]; vs=[]
    for i,p in enumerate(pts):
        d=pts[min(i+1,len(pts)-1)]-pts[max(0,i-1)]; d.z=0; d.normalize()
        side=Vector((-d.y,d.x,0))*(r['width']/2+.55)
        for v in [p-side,p+side]: vs.append((v.x,v.y,v.z-.08))
        for v in [p-side*1.13,p+side*1.13]: vs.append((v.x,v.y,bottom))
    center_base=len(vs); vs.extend([(p.x,p.y,p.z-.08) for p in pts])
    fs=[]; cs=[]
    for i in range(len(pts)-1):
        a=i*4; b=a+4
        c=center_base+i
        fs += [(a,b,c+1,c),(c,c+1,b+1,a+1),(a+2,a+3,b+3,b+2),(a,a+2,b+2,b),(a+1,b+1,b+3,a+3)]; cs += [1,1,0,0,0]
    end=(len(pts)-1)*4
    fs += [(0,1,3,2),(end,end+2,end+3,end+1)]; cs += [0,0]
    mesh('Terrain_RampBanks',vs,fs,cs,True)
...
path('Docks',[(-12.5,-20,1.23),(-9,-21,1.23),(-1.5,-21,1.23)],3.6,4,.35)
...
(OUT/'LighthouseIsland_Routes.json').write_text(json.dumps({'routes':[
    {'name':name,'width':r['width'],'points':[dict(zip(('x','y','z'),p)) for p in r['points']]}
    for name,r in routes.items()]},indent=2),encoding='utf8')
...
scene.render.filepath=str(PREVIEW/'LighthouseIsland_Preview.png')
...
scene.render.filepath=str(PREVIEW/'LighthouseIsland_Harbour.png')
if '--no-render' not in sys.argv: bpy.ops.render.render(write_still=True)
